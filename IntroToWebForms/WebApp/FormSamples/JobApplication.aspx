    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div>
                ...
                <asp:RequiredFieldValidator ID="ValidateName" runat="server" Display="None" ControlToValidate="FullName" ErrorMessage="Name is required."/>
                <asp:RegularExpressionValidator ID="ValidateEmail" runat="server" Display="None" ControlToValidate="EmailAddress" ErrorMessage="Email must be in correct format: " ValidationExpression="\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"/>
                <asp:RegularExpressionValidator ID="ValidatePhoneNumber" runat="server" Display="None" ControlToValidate="PhoneNumber" ErrorMessage="Phone is required, and must be in the correct format: (###)###-#### &nbsp;&nbsp;" ValidationExpression="\(\d{3}\)\s?\d{3}-\d{4}"/>
                <asp:RequiredFieldValidator ID="ValidateJobStatus" runat="server" Display="None" ControlToValidate="FullOrPartTime" ErrorMessage="Full/Part Time must be chosen (It's required)"/>
